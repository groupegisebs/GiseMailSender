    ...
    IBackgroundJobScheduler jobScheduler) : IEmailQueueService
{
    ...
    public async Task<SendMailResponse> QueueEmailAsync(SendMailRequest request, Guid clientId, string? ip, CancellationToken ct = default)
    {
        ...
        var template = await ResolveTemplateAsync(requestedTemplateCode, requestedLanguage, ct);

        if (template is null)
        {
            template = CreateAutoTemplate(requestedTemplateCode, client.ClientCode, requestedLanguage);
    ...
    private async Task<EmailTemplate?> ResolveTemplateAsync(string templateCode, string language, CancellationToken ct)
    {
        var exact = await db.EmailTemplates
            .FirstOrDefaultAsync(t => t.TemplateCode == templateCode && t.Language == language, ct);
        if (exact is not null)
            return exact;

        if (!string.Equals(language, "fr", StringComparison.OrdinalIgnoreCase))
        {
            var fallback = await db.EmailTemplates
                .FirstOrDefaultAsync(t => t.TemplateCode == templateCode && t.Language == "fr", ct);
            if (fallback is not null)
                return fallback;
        }

        // Compat anciennes bases : une seule ligne sans variante langue stricte.
    ...
    private static EmailTemplate CreateAutoTemplate(string templateCode, string clientCode, string language) => new()
    {
        TemplateCode = templateCode,
        Name = $"{EmailTemplate.AutoGeneratedNamePrefix}{clientCode} - {templateCode}",
