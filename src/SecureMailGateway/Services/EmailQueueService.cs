    ...
    IBackgroundJobScheduler jobScheduler) : IEmailQueueService
{
    ...
    public async Task<SendMailResponse> QueueEmailAsync(SendMailRequest request, Guid clientId, string? ip, CancellationToken ct = default)
    {
        ...
        var seed = EmailTemplateCatalog.Find(requestedTemplateCode, requestedLanguage);

        if (template is null)
        {
            // Préférer le contenu seedé (ex. EXPERT_INVITE) plutôt qu'un stub « Template auto-cree ».
            template = seed is not null
                ? EmailTemplateCatalog.ToEntity(seed)
                : CreateAutoTemplate(requestedTemplateCode, client.ClientCode, requestedLanguage);
            ...
            db.EmailTemplateVersions.Add(new EmailTemplateVersion
            {
                ...
                CreatedBy = seed is not null ? "system:seed-on-send" : "system:auto"
            });

            await db.SaveChangesAsync(ct);
            await auditService.LogAsync(
                AuditAction.TemplateCreated,
                userId: seed is not null ? "system:seed-on-send" : "system:auto",
                clientId: clientId,
                ...
                details: new
                {
                    ...
                    Reason = seed is not null
                        ? "Created from catalog seed on first API send"
                        : "Auto-created stub from first API send call"
                },
                ct: ct);
        }
        else if (seed is not null && EmailTemplateCatalog.IsAutoGeneratedStub(template))
        {
            // Stub déjà en base (prod) → remplacer immédiatement avant l'envoi.
            EmailTemplateCatalog.ApplySeed(template, seed);
            db.EmailTemplateVersions.Add(new EmailTemplateVersion
            {
                EmailTemplateId = template.Id,
                ...
                HtmlBody = template.HtmlBody,
                TextBody = template.TextBody,
                CreatedBy = "system:replace-auto-stub"
            });
            await db.SaveChangesAsync(ct);
        }
